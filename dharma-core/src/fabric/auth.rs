...
impl CapToken {
    ...
    fn base_entries(&self) -> Vec<(Value, Value)> {
        ...
        let ops = Value::Array(self.ops.iter().map(|o| Value::Text(o.as_str().to_string())).collect());
        let actions = Value::Array(self.actions.iter().map(|a| Value::Text(a.clone())).collect());
        let queries = Value::Array(self.queries.iter().map(|q| Value::Text(q.clone())).collect());
        let flags = Value::Array(self.flags.iter().map(|f| Value::Text(f.as_str().to_string())).collect());
        ...
        let mut entries = vec![
            ...
            (Value::Text("id".to_string()), Value::Bytes(self.id.to_vec())),
            (
            ...
            (Value::Text("level".to_string()), Value::Text(self.level.clone())),
            ...
            (Value::Text("nbf".to_string()), Value::Integer(self.nbf.into())),
            (Value::Text("exp".to_string()), Value::Integer(self.exp.into())),
        ];
    ...
    pub fn from_value(value: &Value) -> Result<Self, DharmaError> {
        ...
        let id = parse_bytes32(map_get(map, "id").ok_or_else(|| DharmaError::Validation("missing id".to_string()))?)?;
        let issuer_bytes =
            crate::value::expect_bytes(map_get(map, "issuer").ok_or_else(|| DharmaError::Validation("missing issuer".to_string()))?)?;
        let issuer = IdentityKey::from_slice(&issuer_bytes)?;
        let domain = expect_text(map_get(map, "domain").ok_or_else(|| DharmaError::Validation("missing domain".to_string()))?)?;
        let level = expect_text(map_get(map, "level").ok_or_else(|| DharmaError::Validation("missing level".to_string()))?)?;
        ...
        let scopes_val = map_get(map, "scopes").ok_or_else(|| DharmaError::Validation("missing scopes".to_string()))?;
        let scopes = parse_array(scopes_val, Scope::from_value)?;
        let ops_val = map_get(map, "ops").ok_or_else(|| DharmaError::Validation("missing ops".to_string()))?;
        let ops = parse_array(ops_val, Op::from_value)?;
        let actions_val = map_get(map, "actions").ok_or_else(|| DharmaError::Validation("missing actions".to_string()))?;
        let actions = parse_text_array(actions_val)?;
        let queries_val = map_get(map, "queries").ok_or_else(|| DharmaError::Validation("missing queries".to_string()))?;
        let queries = parse_text_array(queries_val)?;
        let flags_val = map_get(map, "flags").ok_or_else(|| DharmaError::Validation("missing flags".to_string()))?;
        let flags = parse_array(flags_val, Flag::from_value)?;
        let oracles_val = map_get(map, "oracles").ok_or_else(|| DharmaError::Validation("missing oracles".to_string()))?;
        let oracles = parse_array(oracles_val, OracleClaim::from_value)?;
        let constraints_val =
            map_get(map, "constraints").ok_or_else(|| DharmaError::Validation("missing constraints".to_string()))?;
        let constraints = parse_array(constraints_val, Constraint::from_value)?;
        let nbf = expect_uint(map_get(map, "nbf").ok_or_else(|| DharmaError::Validation("missing nbf".to_string()))?)?;
        let exp = expect_uint(map_get(map, "exp").ok_or_else(|| DharmaError::Validation("missing exp".to_string()))?)?;
...
impl Scope {
    ...
    fn from_value(value: &Value) -> Result<Self, DharmaError> {
        let map = crate::value::expect_map(value)?;
        let kind = expect_text(map_get(map, "t").ok_or_else(|| DharmaError::Validation("missing scope type".to_string()))?)?;
        let val = map_get(map, "v").ok_or_else(|| DharmaError::Validation("missing scope value".to_string()))?;
        match kind.as_str() {
...
impl OracleClaim {
    ...
    fn from_value(value: &Value) -> Result<Self, DharmaError> {
        let map = crate::value::expect_map(value)?;
        let name = expect_text(map_get(map, "name").ok_or_else(|| DharmaError::Validation("missing oracle name".to_string()))?)?;
        let domain = expect_text(map_get(map, "domain").ok_or_else(|| DharmaError::Validation("missing oracle domain".to_string()))?)?;
        let mode_text = expect_text(map_get(map, "mode").ok_or_else(|| DharmaError::Validation("missing oracle mode".to_string()))?)?;
        let timing_text = expect_text(
            map_get(map, "timing").ok_or_else(|| DharmaError::Validation("missing oracle timing".to_string()))?,
        )?;
        Ok(Self {
            name,
            domain,
...
impl Constraint {
    ...
    fn from_value(value: &Value) -> Result<Self, DharmaError> {
        let map = crate::value::expect_map(value)?;
        let key = expect_text(map_get(map, "k").ok_or_else(|| DharmaError::Validation("missing constraint key".to_string()))?)?;
        let value = map_get(map, "v").ok_or_else(|| DharmaError::Validation("missing constraint value".to_string()))?;
        Ok(Self {
            key,
            value: value.clone(),
        })
