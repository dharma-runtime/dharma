...
pub fn ingest_object(
    ...
    if env_is_verbose() {
        eprintln!(
            "ingest_object: processing {} ({} seq={} sub={})",
            assertion_id.to_hex(),
    ...
    if let Some(base_ref) = overlay_ref {
        ...
        match structural_validate(&assertion, prev_assertion.as_ref())? {
            StructuralStatus::Reject(reason) => {
                eprintln!(
                    "ingest_object: structural reject for {}: {}",
                    assertion_id.to_hex(),
                    reason
                );
        ...
        match validate_action_contract(
            ...
            Err(err) => {
                eprintln!(
                    "ingest_object: contract validation err for {}: {:?}",
                    assertion_id.to_hex(),
                    err
                );
                return Err(err);
    ...
    if is_action {
        ...
        match structural_validate(&assertion, prev_assertion.as_ref())? {
            StructuralStatus::Reject(reason) => {
                eprintln!(
                    "ingest_object: action structural reject for {}: {}",
                    assertion_id.to_hex(),
                    reason
                );
                return Err(IngestError::Validation(reason));
        ...
        match validate_action_contract(store, index, &subject, &assertion, assertion_id, None) {
            ...
            Err(err) => {
                eprintln!(
                    "ingest_object: action contract validation err for {}: {:?}",
                    assertion_id.to_hex(),
                    err
                );
                return Err(err);
    ...
    match structural_validate(&assertion, prev_assertion.as_ref())? {
        StructuralStatus::Reject(reason) => {
            eprintln!(
                "ingest_object: generic structural reject for {}: {}",
                assertion_id.to_hex(),
                reason
            );
            return Err(IngestError::Validation(reason));
    ...
    match validate_generic_contract(store, &assertion) {
        ...
        Err(err) => {
            eprintln!(
                "ingest_object: generic contract validation err for {}: {:?}",
                assertion_id.to_hex(),
                err
            );
            return Err(err);
    ...
    if env_is_verbose() {
        eprintln!(
            "ingest_object: accepted assertion {}",
            assertion_id.to_hex()
        );
    }
    metrics::assertions_ingested_inc();
